...
class nfw(potential):
  def __init__(self, rhos=None, rs=None, mvir=None, rvir=None, c=None, Msun=False, interp=False):
    self.t_cur = None
    self.ttab = None
    self.ztab = None
    self.mvirtab = None
  ...
  def evaluate_at_time(self,t):
    ...
    if self.ttab is None:
      subprocess.call([r'/Users/users/gronnow/src/evolzhao', str(self.mvir*24393036.0465)]) #Run external programme to generate table for given mass
      self.ttab,self.ztab,self.mvirtab,self.conctab,self.rvirtab=np.loadtxt("haloevol.dat",skiprows=1,unpack=True)
